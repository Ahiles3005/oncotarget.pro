...
<? endif; ?>
<form id="f_feedback_<?= $ALX ?>" name="f_feedback_<?= $ALX ?>" action="<?= POST_FORM_ACTION_URI ?>" method="post"
            ...
            <? if (count($arResult["TYPE_QUESTION"]) >= 1): ?>
                ...
                <div class="afbf_item_pole required is_filled">
                    ...
                    <div class="afbf_inputtext_bg">
                        <? $typeQuestionName = "";
                        if (isset($_POST["type_question_name_" . $ALX])) $typeQuestionName = trim(htmlspecialcharsEx($_POST["type_question_name_" . $ALX])); ?>
                        <input type="hidden" id="type_question_name_<?= $ALX ?>" name="type_question_name_<?= $ALX ?>"
                               value="<?= (strlen($typeQuestionName) > 0) ? $typeQuestionName : $arResult["TYPE_QUESTION"][0]["NAME"]; ?>">
...
//            }
                $strFBtext .= '
                                        <div class="afbf_inputtext_bg" id="error_EMPTY_TEXT">
                                                <textarea class="contacts__form-textarea" cols="10" rows="10" id="EMPTY_TEXT' . $ALX . '" name="FEEDBACK_TEXT_' . $ALX . '" placeholder="' . GetMessage("ALX_TP_MESSAGE_TEXTMESS") . '">' . $arResult["FEEDBACK_TEXT"] . '</textarea>
                                        </div>
                                        <div class="afbf_error_text">' . GetMessage("AFBF_ERROR_TEXT") . '</div>
                                </div>';

            } ?>
            <div class="dantrolen_form_wrapper">
				<div class="dantrolen_form_title">Закупка дантролена</div>
                <?
                ...
                foreach ($arResult["FIELDS"] as $key => $arField):?>
                    ...
                    ?>
                    <div id="afbf_<?= mb_strtolower($arField["CODE"]) ?>" class="<?= $fieldClass ?>">
                        <? /*LIST*/
                        ?>
                        <? if ($arField["TYPE"] == "L"): ?>
                                        ...
                                        <? if (!isset($_POST["FIELDS"][$arField["CODE"]]) && !isset($arResult["FORM_ERRORS"]["EMPTY_FIELD"][$arField["CODE"]])): ?>
                                            <div class="afbf_checkbox<?= $cAddClass ?>">
                                                <label for="<?= $arField["CODE"] . '_' . $v["ID"] ?>">
                                                    <input id="<?= $arField["CODE"] . '_' . $v["ID"] ?>" type="checkbox"
                                                           name="FIELDS[<?= $arField["CODE"] ?>][]"
                                                           value="<?= $v["ID"] ?>" <? if ($v["DEF"] == "Y") echo 'checked="checked"'; ?>>
                                        ...
                                        <? else: ?>
                                            <div class="afbf_checkbox<?= $cAddClass ?>">
                                                <label for="<?= $arField["CODE"] . '_' . $v["ID"] ?>">
                                                    <input id="<?= $arField["CODE"] . '_' . $v["ID"] ?>" type="checkbox"
                                                           name="FIELDS[<?= $arField["CODE"] ?>][]"
                                                           value="<?= $v["ID"] ?>" <? if (in_array($v['ID'], $_POST["FIELDS"][$arField["CODE"]])) echo 'checked="checked"'; ?>>
                                                    <span class="afbf_checkbox_box">
                                                                                                        <span class="afbf_checkbox_check"></span>
                                                                                                </span><?= $v["VALUE"] ?>
                                                </label><br/>
                                ...
                                <? else: ?>
                                    <div class="afbf_radio">
                                        <? foreach ($arField["ENUM"] as $v): ?>
                                            <? if (!isset($_POST["FIELDS"][$arField["CODE"]]) && !isset($arResult["FORM_ERRORS"]["EMPTY_FIELD"][$arField["CODE"]])): ?>
                                                <label for="<?= $arField["CODE"] . '_' . $v["ID"] ?>">
                                                    <input id="<?= $arField["CODE"] . '_' . $v["ID"] ?>" type="radio"
                                                           name="FIELDS[<?= $arField["CODE"] ?>]"
                                                           value="<?= $v["ID"] ?>" <? if ($v['DEF'] == 'Y') echo 'checked="checked"'; ?>>
                                            ...
                                            <? else: ?>
                                                <label for="<?= $arField["CODE"] . '_' . $v["ID"] ?>">
                                                    <input id="<?= $arField["CODE"] . '_' . $v["ID"] ?>" type="radio"
                                                           name="FIELDS[<?= $arField["CODE"] ?>]"
                                                           value="<?= $v["ID"] ?>" <? if ($v['ID'] == $_POST["FIELDS"][$arField["CODE"]]) echo 'checked="checked"'; ?>>
                                                    <span class="afbf_radio_circle"></span>
                                                    <span class="afbf_radio_check"></span>
                                                    <? echo $v["VALUE"] ?></label><br/>
                                            <? endif; ?>
                        ...
                        <? elseif ($arField["USER_TYPE"] == "HTML"): ?>
                            <div class="afbf_inputtext_bg" id="error_<?= $arField["CODE"] ?>">
                                <? if (!empty($_POST["FIELDS"][$arField["CODE"]])): ?>
                                    <textarea cols="" rows="" id="<?= $arField["CODE"] ?>1"
                                              name="FIELDS[<?= $arField["CODE"] ?>]"
                                              style="height:<?= $arField["USER_TYPE_SETTINGS"]["height"] ?>px;"><?= trim(htmlspecialcharsEx($_POST["FIELDS"][$arField["CODE"]])) ?></textarea>
                                <? elseif (!empty($arField["AUTOCOMPLETE_VALUE"])): ?>
                                    <textarea cols="" rows="" id="<?= $arField["CODE"] ?>1"
                                              name="FIELDS[<?= $arField["CODE"] ?>]"
                                              style="height:<?= $arField["USER_TYPE_SETTINGS"]["height"] ?>px;"><?= trim(htmlspecialcharsEx($arField["AUTOCOMPLETE_VALUE"])) ?></textarea>
                                <? else: ?>
                                    <textarea cols="" rows="" id="<?= $arField["CODE"] ?>1"
                                              name="FIELDS[<?= $arField["CODE"] ?>]"
                                              style="height:<?= $arField["USER_TYPE_SETTINGS"]["height"] ?>px;"
                                              onblur="if(this.value==''){this.value='<?= $arField["DEFAULT_VALUE"]["TEXT"] ?>'}"
                                              onclick="if(this.value=='<?= $arField["DEFAULT_VALUE"]["TEXT"] ?>'){this.value=''}"><?= $arField["DEFAULT_VALUE"]["TEXT"] ?></textarea>
                                <? endif; ?>
                                <? if ($arField['REQUIRED'] == 'Y'): ?>
                                    <div class="afbf_error_text"><?= GetMessage('AFBF_ERROR_TEXT') ?></div>
                                <? endif ?>
                        ...
                        elseif ($arField["USER_TYPE"] == "Date" || $arField["USER_TYPE"] == "DateTime"):?>
                            <div class="afbf_inputtext_bg afbf_inputtext_bg_calendar"
                                 id="error_<?= $arField["CODE"] ?>"><?
                                ...
                                <? if (!empty($_POST["FIELDS"][$arField["CODE"]])): ?>
                                    <input type="text" size="40" id="<?= $arField["CODE"] ?>1"
                                           name="FIELDS[<?= $arField["CODE"] ?>]"
                                           value="<?= trim(htmlspecialcharsEx($_POST["FIELDS"][$arField["CODE"]])) ?>"
                                           class="afbf_inputtext" readonly="readonly"
                                           onclick="BX.calendar({node:this,field:'FIELDS[<?= $arField["CODE"] ?>]',form:'',bTime:<?= $bShowTime ?>,currentTime:'<?= (time() + date("Z") + CTimeZone::GetOffset()) ?>',bHideTime:<?= $bHideTime ?>});"/>
                                <? else: ?>
                                    <input type="text" size="40" id="<?= $arField["CODE"] ?>1"
                                           name="FIELDS[<?= $arField["CODE"] ?>]"
                                    ...
                                    <? foreach ($arField["LINKED_ELEMENTS"] as $arEl): ?>
                                        <p class="afbf_checkbox">
                                            <label for="<?= $arField["CODE"] ?>1_<?= $arEl["ID"] ?>">
                                                <input type="checkbox" name="FIELDS[<?= $arField["CODE"] ?>][]"
                                                       value="<?= $arEl["ID"] ?>"
                                                       id="<?= $arField["CODE"] ?>1_<?= $arEl["ID"] ?>" <?
                                    ...
                                    <? if (!empty($arField["LINKED_ELEMENTS"])):
                                        foreach ($arField["LINKED_ELEMENTS"] as $val):?>
                                            <label for="<?= $arField["CODE"] . '_' . $val["ID"] ?>">
                                                <input id="<?= $arField["CODE"] . '_' . $val["ID"] ?>" type="radio"
                                                       name="FIELDS[<?= $arField["CODE"] ?>]"
                                                       value="<?= $val["ID"] ?>" <? if ($val['ID'] == $_POST["FIELDS"][$arField["CODE"]]) echo 'checked="checked"'; ?>>
                                                <span class="afbf_radio_circle"></span>
                                                <span class="afbf_radio_check"></span>
                        ...
                        <? elseif ($arField["TYPE"] == "G"): ?>
                            <div class="afbf_inputtext_bg is_filled" id="error_<?= $arField["CODE"] ?>">
                                <? if (!empty($arField["LINKED_SECTIONS"])):
                                    ?>
                                    <select class="<? if ($arField["PROPERTY"]["MULTIPLE"] != "Y"): ?>afbf_select<? else: ?>afbf_inputtext afbf_select_sect<? endif; ?>"
                                            name="FIELDS[<?= $arField["CODE"] ?>][]"<? echo(!empty($arField["PROPERTY"]["MULTIPLE_CNT"]) && $arField["PROPERTY"]["MULTIPLE"] == "Y" ? ' size="' . $arField["PROPERTY"]["MULTIPLE_CNT"] . '"' : ""); ?><?
                            ...
                            <div class="afbf_inputtext_bg" id="error_<?= $arField["CODE"] ?>">
                                <? if (!empty($_POST["FIELDS"][$arField["CODE"]])): ?>
                                    <input type="text" size="40" id="<?= $arField["CODE"] ?>1"
                                           name="FIELDS[<?= $arField["CODE"] ?>]"
                                           value="<?= trim(htmlspecialcharsEx($_POST["FIELDS"][$arField["CODE"]])) ?>"
                                           class="afbf_inputtext contacts__form-input"
                                           placeholder="<? echo $arField["NAME"] ?><? if ($arField["REQUIRED"]): ?>*<? endif ?>"/>
                                <? elseif (!empty($arField["AUTOCOMPLETE_VALUE"])):
                                    $readonly = "";
                                    ...
                                    <input type="text" size="40" id="<?= $arField["CODE"] ?>1"
                                           name="FIELDS[<?= $arField["CODE"] ?>]"
                                           <?= $readonly ?>value="<?= trim(htmlspecialcharsEx($arField["AUTOCOMPLETE_VALUE"])) ?>"
                                           class="afbf_inputtext contacts__form-input"
                                           placeholder="<? echo $arField["NAME"] ?><? if ($arField["REQUIRED"]): ?>*<? endif ?>"/>
                                <? else: ?>
                                    <input type="text" size="40" id="<?= $arField["CODE"] ?>1"
                        ...
                        <? elseif ($arField["TYPE"] == "F"): ?>
                            <? $k++; ?>
                            <input type="hidden" name="codeFileFields[<?= $arField['CODE'] ?>]"
                                   value="<?= $arField['CODE'] ?>">
                            <div class="afbf_inputtext_bg file">
                                <input type="hidden" name="FIELDS[myFile][<?= $arField["CODE"] ?>]">
                                <? if ($arField["MULTIPLE"] == "Y"): ?>
                                    <input type="file" id="afbf_file_input_add<?= $k ?>"
                                           name="myFile[<?= $arField['CODE'] ?>][]" class="afbf_file_input_add"
                                           multiple="true"/>
                                <? else: ?>
                                    <input type="file" id="afbf_file_input_add<?= $k ?>"
                                           name="myFile[<?= $arField['CODE'] ?>]"
                                           class="afbf_file_input_add"/>
                                <? endif; ?>
                                <div class="afbf_input_group">
                                    <input type="text" size="40" id="<?= $arField["CODE"] ?>1"
                                           name="FIELDS[myFile][<?= $arField["CODE"] ?>]" value=""
                ...
                <? if ($arParams["USE_CAPTCHA"]): ?>
                    <? if ($arParams["CAPTCHA_TYPE"] == 'default'): ?>
                        <div class="afbf_item_pole item_pole__captcha required">
                            ...
                            <div class="afbf_inputtext_bg"><input type="text" class="afbf_inputtext"
                                                                  id="captcha_word_<?= $ALX ?>" name="captcha_word"
                                                                  size="30"
                    ...
                    <? endif; ?>
                    <? elseif ($arParams["CAPTCHA_TYPE"] == 'recaptchav3'): ?>
                    ...
                    <input type="hidden" id="g-recaptcha-response" name="g-recaptcha-response"/>
                        ...
                        <script type="text/javascript">
                            ...
                            var AltasibFeedbackResetCaptcha_<?=$ALX?> = function () {

                                grecaptcha.ready(function () {
                                    grecaptcha.execute('<?=$arResult["SITE_KEY"];?>').then(function (token) {
                                ...
                                $("#fb_close_<?= $ALX ?>")[0].onclick = (e) => {
                                    e.preventDefault();
                                    var asd = grecaptcha.execute('<?=$arResult["SITE_KEY"];?>').then(function (token) {
                                        var recaptchaField = $("#g-recaptcha-response")[0];
                                        if (recaptchaField) {
                                            recaptchaField.value = token;
                                        }
                                    ...
                                    asd.then(() => {
                                        $("#f_feedback_<?= $ALX ?>")[0].requestSubmit();
                                    });
                                };

                            };
                            <?                          if($arParams['ALX_LINK_POPUP'] == 'Y'):?>
                            $.getScript('https://www.google.com/recaptcha/api.js?onload=AltasibFeedbackResetCaptcha_<?=$ALX?>&render=<?=$arResult["SITE_KEY"]?>&hl=<?=LANGUAGE_ID?>')
                                .fail(function (jqxhr, settings, exception) {
                                    console.log('Error loading google :)')
                                });
                            <?                          else:?>
                ...
                ?>
                <? if (CSite::InDir('/en/')): ?>
                    <div class="contacts__form-privacy">By clicking on the button, you consent your personal data being
                        processed and agree to the <a href="#">privacy policy</a>
                    </div>
                <? else: ?>
                    <div class="contacts__form-privacy">Нажимая на кнопку, вы даете согласие на
                        обработку персональных данных и соглашаетесь <a href="#">c политикой
                            конфиденциальности</a>
                    </div>
                <? endif; ?>
			</div>
        </div>
        <div class="dantrolen_button">
